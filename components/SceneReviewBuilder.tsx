...
type SceneReviewBuilderProps = {
  ...
  appMode?: AppMode;
  hasAutomaticMatchReport?: boolean;
  ...
  return (
    <div
      className={`rounded-xl border p-3 ${
        primary ? "border-zinc-300 bg-zinc-100" : "border-zinc-200 bg-white"
      }`}
    >
...
export default function SceneReviewBuilder({
  ...
  const shouldCollapseSupportEvidence =
    appMode !== "debug" && hasAutomaticMatchReport;

  const supportEvidencePanels = (
    <div className="space-y-3">
      ...
      <EvidenceDetails
        ...
        description="짧은 클립에서 직접 입력할 폼의 후보 값을 준비합니다."
      >
        {videoDraftPanel}
      </EvidenceDetails>

      <EvidenceDetails
        id={shouldCollapseSupportEvidence ? undefined : "riot-evidence-section"}
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm leading-6 text-zinc-500">
          Riot 매치를 연결하면 장면을 자동으로 찾아드립니다. 자동 후보가 부족할
          때만 직접 입력하세요.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-4">
        ...
        <p className="mt-3 text-xs leading-5 text-zinc-500">
          영상/Riot 결과는 최종 판단이 아니라 자동 후보와 직접 입력을 보조하는
          근거입니다.
        </p>
          ...
          <summary className="cursor-pointer list-none px-4 py-3 marker:hidden">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-zinc-500">
                  자동 후보가 이상하거나 직접 시간을 확인하고 싶을 때만
                  사용하세요.
                </p>
              </div>
              <span className="rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-medium text-zinc-500 group-open:hidden">
